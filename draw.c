...
void add_sphere( struct matrix * points, 
  ...
  for(c = 0; c < temp->lastcol; c++){
    add_edge(points,temp->m[0][c],temp->m[1][c],temp->m[2][c],temp->m[0][c],temp->m[1][c],temp->m[2][c]);
  }
  free(temp);
}
...
void generate_sphere( struct matrix * points, 
  ...
  for(p = 0; p < 1; p+=1/step){
    for(t = 0; t < 1; t+=1/step){
      add_point(points, r * cos(M_PI * t) + cx, r * sin(M_PI * t) * cos(2 * M_PI * p) + cy, r * sin(M_PI * t) * sin(2 * M_PI * p));
    }
  }
}    
...
void generate_torus( struct matrix * points, 
  ...
  for(p = 0; p < 1; p+=1/step){
    for(t = 0; t < 1; t+=1/step){
      add_point(points, (r1 * sin(2 * M_PI * t) + r2) * cos(2 * M_PI * p) + cx, r1 * cos(2 * M_PI * t) + cy, (r1 * sin(2 * M_PI * t) + r2) * sin(2 * M_PI * p));
    }
  }
}
